...
class ParticipantTracker:
    ...
    def add_known_names(self, names: list[str]) -> None:
        ...
        Dedupe is CASE-INSENSITIVE: Meet shows "sayem biswas" in one place and
        "Sayem Biswas" in another; they must count as one person.
        """
    ...
    def _diff_and_record(self, present: set[str]) -> None:
        # Case-insensitive diff: the same person must not generate phantom
        # join/leave events just because Meet changed the casing of the name.
        current_lower = {n.lower(): n for n in self._current}
        present_lower = {n.lower(): n for n in present}
        joined = [present_lower[k] for k in present_lower.keys() - current_lower.keys()]
        left = [current_lower[k] for k in current_lower.keys() - present_lower.keys()]
        if joined or left:
    ...
    async def _read_participants(self) -> set[str]:
        ...
        Full-name aria-labels ("Profile photo of <Full Name>") are included
        because tile text can be truncated.
        """
        names: set[str] = set()
        seen_lower: set[str] = set()
        # Ordered from most-specific to least-specific.
        selectors = [
            # Full-name sources ("Profile photo of Sayem Biswas").
